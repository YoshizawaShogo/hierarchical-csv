...
| D-018 | 2026-07-19 | グロブの **OR はブレース展開 `{a,b}`** で表現（正規表現の `|` ではない）。`sheet` / `*_optional` など全グロブ箇所で有効 | 型を絞った分、パターン表現力をグロブ側で補う。ブレースはグロブ標準で `*` の直感に沿う |
...
- `*`（任意文字列）等の標準グロブに加え、**OR はブレース展開 `{a,b}`** で表す（正規表現の `|` ではない）。
...
- `type` と他規則の組合せ検証順序（例: `int` かつ `pattern` の扱い）
