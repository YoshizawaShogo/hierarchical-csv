...
| D-022 | 2026-07-19 | グロブの **OR は `|`（＋ `()` グループ化）** に変更（D-018 のブレース `{a,b}` を差し替え）。例: `metric_(cpu\|gpu)`。標準グロブ機能ではないためマッチ前に複数グロブへ展開 | `\|` の方が択一として直感的、との判断 |
...
フック（`.hooks.toml`）まわりの詰め（D-024 に紐づく）:

| ID | 論点 | 現時点の推奨 | 状態 |
|----|------|--------------|------|
| Q-5 | 内蔵 Constraint 検証と `on_save` フックの役割分担・順序 | 内蔵制約を先に検証 → 通れば on_save フック（フックは追加のカスタム処理） | 未決 |
| Q-6 | フック実行の信頼モデル | `when_py` と同じプロジェクト信頼ゲートを共有 | 未決 |
| Q-7 | `fail_policy` の値セット | `error`（中止）/ `warn`（警告のみ）/ `ignore` の3種 | 未決 |
| Q-8 | フックの CWD と変数のパス形式 | CWD=プロジェクトルート、`{csv_path}` 等は絶対パス | 未決 |
| Q-9 | プロジェクト設定（旧 config.yaml の `project`）の置き場 | 別 `.project.toml` を用意 or 不要（自動検出）で決める | 未決 |
...
- `*`（任意文字列）等の標準グロブに加え、**OR は `|`（択一）** で表す。`()` でグループ化可。
  - 例: `metric_(cpu|gpu)` → `metric_cpu` / `metric_gpu`、`modules/(auth|db).csv` → 2ファイルに適用。
  - 実装: `|` / `()` は標準グロブ機能ではないため、マッチ前に複数の具体グロブへ展開して扱う。
...
`when` の宣言式で足りない複雑な条件のために、**Python エスケープハッチ** `when_py` を用意する。

- **L1: `when`** … 上記の宣言式（`{}` + `&`/`|`）。安全・依存なし。既定はこちら。
- **L2: `when_py`** … **bool を返す Python 式**（opt-in・強力）。注入変数:
  - `$column` … その行のセル値を列名で引ける辞書（例: `$column['processor_type']`）
  - `$row` … その行のラベル（行ヘッダー時）または index
  - 実装上は Python 変数 `column` / `row`（辞書）として束縛し、`$` は見た目のエイリアス（文字列置換はしない＝注入対策）。
...
column   = "cooling"
required = true
when_py  = "$column['processor_type']=='ARM' and int($column['cores'])>=4"
```

**Python 実行の安全既定（重要）:**
- **プロジェクト信頼が有効なときだけ実行**する（`.constraints.toml` は設計者が書き、エンドユーザーが開くため、任意コード実行の経路になる）。フック（config.yaml）と**同じ信頼モデルを共有**。
- **Python が PATH に無ければ `when_py` はスキップ＋警告**。L1 の宣言式は Python 非依存で常に動作。
- **バッチ評価**（行ごとに Python を起動せず、シート単位で1回渡して該当行を返す）で性能を担保。

### 6.8 実装スコープ

- **最初（Phase 1〜2）**: `header` と、`value` の単純な規則（`required` / `enum` / `pattern` / `min` / `max` / `type`）を実装。
- **後から**: `value` の `when`（L1 宣言式の条件付き）を追加。
- **さらに後**: `when_py`（L2 Python）を追加。信頼ガード・Python 依存・バッチ評価が必要なため優先度は最後。カテゴリ（`header`/`value`）は変えず、`value` にフィールドを足すだけで拡張できる器にしておく。
...
- 保存時の順序: **内蔵 Constraint 検証 →（通れば）`on_save` フック → 完了**（Q-5 で確定予定）。

---

## 8. フック（`.hooks.toml`）

> D-024 に基づく。旧仕様の `config.yaml` から **`.hooks.toml`（TOML）** に変更。構造はサンプル踏襲。詳細な挙動は 3章 Q-5〜Q-9 で確定予定。
...
fail_policy = "error"          # error / warn / ignore（Q-7）
...
- 変数: `{csv_path}` / `{project_root}` / `{output_path}`。
- フックはサブプロセス実行 ＝ コード実行。`when_py` と同じ**プロジェクト信頼ゲートを共有**する想定（Q-6）。
