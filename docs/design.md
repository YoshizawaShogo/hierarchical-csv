...
| ID | 論点 | 選択肢 | 現時点の推奨 | 状態 |
|----|------|--------|--------------|------|
| Q-1 | Git 操作の実装方法 | (a) `git` CLI 直叩き / (b) `git2`(libgit2) crate | (a) CLI 直叩きで開始し、必要なら後で(b)へ | 未決 |
| Q-2 | CSV パーサ | (a) `csv` crate / (b) 自前 split | (a) `csv` crate | 未決 |
| Q-3 | 最初のコミットの範囲 | スキャフォールド＋型定義＋LICENSE まで | 同左 | 未決 |
| Q-4 | この環境での検証方針 | Rust=ユニットテスト中心 / フロント=Vitest | 同左 | 未決 |
